...
A generic carries its full subtype, range and all, because that is what the Self-check line
promises and what `ci/entity_stub.py` has to declare. `Entity.generic_lines` strips the range
back off for the figures, which draw the base type alone so the box stays as narrow as every
other module's.
"""
...
RESET_SYNC = Entity(
    ...
    [P("clock"), P("reset_n")],
    [P("reset_s2_n")])
# Both button vectors are COUNT bits wide, so their width is a generic, not a number.
BUTTON_SYNC = Entity(
    "button_sync",
    [P("clock"), P("reset_s2_n"), P("button_n", "COUNT-1:0")],
    [P("button_edge_s2", "COUNT-1:0")],
    generics=[("COUNT", "natural range 1 to 3", "1")])
...
SYNC = Entity(
    ...
    [P("sync_out", "SIZE-1:0")],
    generics=[("SIZE", "natural range 1 to 15", "1"), ("PRESET", "std_logic", "'0'")])
...
COUNTER = Entity(
    ...
    [P("count", label="count (natural)", bus=True, vhdl="natural range 0 to RADIX-1"),
     P("tick")],
    generics=[("RADIX", "natural range 1 to 2**16", "10")])
SIPO8 = Entity(
    "sipo8",
