...
then simulate by hand in CircuitVerse before any VHDL exists. See the
[root README](../README.md) for how those three layers fit together.
...
* **The named traps are worth full marks on their own.** Several parts exist entirely to see whether
  a candidate avoids one specific mistake: reading a scheduled signal as if it had already updated,
  synchronizing a bus one bit at a time, calling metastability a wrong answer rather than a late
  one, a `case` branch that assigns nothing, a state machine driven by a level instead of a pulse,
  and a timer output wired into a clock port. Where a solution flags one of these, a candidate who
