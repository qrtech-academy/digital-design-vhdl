...
Nothing here reads `reset_s2_n` back, so under VHDL-93 it can be driven straight from the process,
as L04 A.5's listing does. A candidate who routes it through an extra internal signal is equally
correct, just carrying one declaration that buys nothing.
...
**Without the synchronous release**, in a design of a dozen flip-flops - worth a comment rather
than a mark, but the sentence the other two marks are really testing for: the reset's rising edge
...
So has one who points out that the surviving assignment is `any_s <= any_s or bits(0);` with
`any_s` absent from the sensitivity list, so what synthesis is being handed is a **combinational
feedback loop**: an output fed back to its own input through a gate, with no flip-flop in the
path. Quartus reports that rather than building it. "Compiles cleanly" here means GHDL analyzes
and simulates it, which is exactly the gap the question is about.
...
on `S2`, a fall-back arrow from `S1` to `S0`, and a re-entry arrow from `S3` to `S1` on `din = 1`.
That is eight arrows, two out of each of the four states. `y = 1` is written **on the `S3` bubble**, not on
