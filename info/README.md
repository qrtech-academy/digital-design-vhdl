...
None of that is taught from scratch. It is refreshed where the language needs it, briefly and in
the appendices rather than in the lectures: L01 restates gate and Boolean notation so the course
has one convention, L02 works a Karnaugh map end to end because minimization is what motivates the
VHDL that follows, and L08 designs one state machine by hand before writing it, because seeing the
state table become a `case` statement is the point of the lecture.
...
* A two-digit hexadecimal 7-segment display, in CircuitVerse and in VHDL
...
* Composing a timer, a synchronizer and a shift register: the walking LED on the DE0-CV
