...
[exercises 8 and 9](./b_exercises.md): `reset_sync` is [A.5](#a5-synchronizing-a-reset-signal-assert-async-release-sync)
above, and `button_sync` is [A.6](#a6-reusing-the-chain-for-edge-detection---and-incidentally-debouncing)
and [A.8](#a8-generics-one-module-several-sizes), both printed in full. Copy your own into
`led_toggle_sync/` to build it.
