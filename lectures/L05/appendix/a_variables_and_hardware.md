...
`parity_s <= parity_s xor bits(0);`, and `parity_s` is not in the sensitivity list: as hardware
that is a **combinational feedback loop**, an output fed back to its own input through a gate with
no flip-flop in the path. GHDL analyzes and simulates it happily, which is how the bug reaches a
simulation at all; Quartus reports the loop rather than building it. The lesson is the same one
either way, and the fix below removes both problems at once.
...
the analyzer has no period of yours to compare anything against, so it warns that no base clocks
were constrained and reports Fmax against a default clock instead: a number, but not one you can
act on. Separately, a design that "fails timing" is one where some path turned
...
and the scheduling rule above, and L08's state machines add only an enumerated type, a `case` on it, and
one new concurrent form, the conditional signal assignment. What changes is what the state is used for.
