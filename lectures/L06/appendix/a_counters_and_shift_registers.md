...
* **Watch `carry_out` on the adder while it happens.** That is the fifth bit, and the drawing shows
  exactly where it ends up: high throughout the single count at which the register rolls over,
  connected to nothing, leaving the adder and stopping there. A wider counter would feed it into a fifth stage;
...
* `clock` to the 50 MHz oscillator, `reset_n` to a push button (into the `reset_sync`, never
  straight into the counter), and `count[27]` down to `count[24]` - the top four bits of the bus
  Quartus builds for the widened integer port, since `count` is a `natural` and not a vector you
  could slice in VHDL - to four LEDs.
