...
**Self-check:** ports `clock`, `reset_s2_n`, `serial_in` (in) and `parallel_out`
(`std_logic_vector(7 downto 0)`, out), declared in that order; its testbench is in
...
**Self-check:** ports `clock`, `reset_s2_n`, `load`, `shift`, `serial_in`, `parallel_in`
(`std_logic_vector(7 downto 0)`) and `serial_out` (out), declared in that order; its testbench is
...
* Change the bit-counter comparison from `7` to `6`.
