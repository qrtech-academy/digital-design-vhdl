...
* `TICK_COUNT = 50,000,000`: timeout once per second.
* `TICK_COUNT = 25,000,000`: twice per second.
* `TICK_COUNT = 5,000,000`: ten times per second.

Strictly, the counter runs `0` through `TICK_COUNT`, so a full period is `TICK_COUNT + 1` cycles.
That one extra tick is between 0.02 and 0.2 ppm at these values, far below the board oscillator's
own tolerance, so the round numbers above are used for clarity; for an exact cycle count use
...
distinction L08's Moore-versus-Mealy discussion turns on.
