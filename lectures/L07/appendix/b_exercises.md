...
**Self-check:** name your entity `timer`, with generic `TICK_COUNT` (`natural`), inputs `clock`,
...
**Self-check:** name your entity `blinker`, with generic `TICK_COUNT` (`natural`), inputs `clock`,
...
**Self-check:** name your entity `walking_led`, with generics `LED_COUNT` and `TICK_COUNT` (both
`natural`), inputs `clock`, `reset_n`, `button_n`, and output `led`, declared in that order; its
