...
**Tip:** Since the state order changed from Gray code to binary, expect **two** transitions where
both state bits flip at once: `01 -> 10` and `11 -> 00`. Per A.2 that is harmless to the machine
itself, since nothing samples the state register between edges, and CircuitVerse's ideal
simulation will not show you anything either way.

It is harmless to `Y` here too, and for a reason worth naming: `Y` is high exactly when `Q1 = 1`,
so it is one flip-flop's output taken straight to the LED, with no gate between them to produce a
glitch while the two bits skew. Contrast A.2's machine, whose `Y = Q1 * Q2'` **is** decoded from
both bits and can therefore glitch briefly on real hardware. Ask yourself what would change here
if the LED lit in only one state, say `10`, and what you would do about it if `Y` were driving
...
* Design it **non-overlapping**: after a full match, start looking for a fresh `1, 0, 1` from
  scratch, rather than reusing the trailing bits. Exercise 4 turns this machine into VHDL and its
  shipped testbench checks for it, so an overlapping design fails there.
...
Write an entity named `uart_rx8`: a receiver for one byte of asynchronous serial data, in a frame
format modelled on a UART's, with one deliberate difference set out below.
...
* **Eight data bits**, most significant first, one bit period each. This is the deliberate
  difference: see the note below the figure.
...
* That error is a fraction of a percent, and it accumulates across a frame. The stop bit is
  sampled 9.5 bit periods after the start edge - half a bit period to reach the middle of the
  start bit, then nine more bit periods. How far has your sample point drifted from the middle
  of the stop bit by then, measured in oversample ticks? How many ticks of margin did it have
  to begin with?
...
**Self-check:** name your entity `uart_tx8`, with generic `BIT_TICK_COUNT` (`natural`, default
`5207`), inputs
